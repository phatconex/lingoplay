...
function generateOptions(word, pool) {
...
function MCQuestion({ word, pool, onCorrect, onWrong }) {
  const [selected, setSelected] = useState(null);
  const [correct, setCorrect] = useState(null);
  const options = useRef(generateOptions(word, pool)).current;
...
function SpellingQuestion({ word, onCorrect, onWrong }) {
  ...
  const check = () => {
    if (locked.current) return;
    locked.current = true;
    speakWord(word.en); // Speak only when user submits
  ...
  return (
    <div className="flex flex-col flex-1 animate-fade-in items-center">
...
export default function ReviewPage() {
  ...
  const [queue, setQueue] = useState([]); // words in order
  const [index, setIndex] = useState(0);
  const [mode, setMode] = useState('mc'); // alternates: mc | spelling
  ...
  const loadPool = async () => {
    setLoading(true);
    // Fetch all vocab items for user
    ...
    // Build interleaved queue: each word appears twice – once MC, once Spelling
    // But we just alternate modes per question, not per word
    setPool(mapped); // full pool for generating MC distractors
  ...
  if (queue.length === 0) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-4 text-center px-8">
        ...
        <h2 className="text-2xl font-extrabold text-[#0D1A63]">Chưa có từ vựng nào</h2>
        <p className="text-slate-500 font-semibold">Hãy thêm từ vựng vào các bộ từ trước nhé!</p>
        <button onClick={() => navigate('/')} className="mt-4 px-8 py-3 bg-[#2845D6] text-white font-bold rounded-2xl shadow-[0_4px_0_#0D1A63] hover:bg-[#1A2CA3] transition-all">
          Về trang chủ
        </button>
      </div>
    );
  ...
  if (done) {
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-6 text-center px-8 animate-fade-in">
        <Trophy size={72} className="text-yellow-400" />
        <h2 className="text-4xl font-extrabold text-[#0D1A63]">Ôn tập hoàn thành! 🎉</h2>
        ...
        <div className="flex gap-4 mt-2">
          <button onClick={() => navigate('/')} className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-2xl transition-all">
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col animate-fade-in p-4 md:p-8 h-full">
      {/* Header */}
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate('/')} className="p-2 rounded-xl text-slate-400 hover:text-[#2845D6] hover:bg-[#E8EBF5] transition-all">
