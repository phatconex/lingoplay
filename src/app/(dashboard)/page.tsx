...
import {
  Book, Clock, Plus, MoreVertical, X, Settings,
  Edit3, Trash2, Flame, BookOpen, Layers
} from 'lucide-react';
...
function getSRSInfo(set: any): SRSInfo {
  ...
  if (!last_studied) {
    return {
      isDue: true, daysRemaining: 0, label: 'Mới',
      sublabel: 'Chưa học lần nào', icon: '✨',
  ...
  if (review_count >= 5) {
    return {
      isDue: false, daysRemaining: null, label: 'Đã thuộc',
      sublabel: 'Hoàn thành 5 lần học', icon: '✅',
...
export default function HomePage() {
  const { user, appData, checkStreak } = useAppContext();
  ...
  const [managingSet, setManagingSet] = useState<any>(null);
  ...
  if (loading) {
    return (
      <div className="w-full h-full flex items-center justify-center animate-fade-in">
        <div className="animate-spin text-[#2845D6]">
          <Clock size={48} />
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full max-w-5xl mx-auto h-full animate-fade-in p-2 md:p-8 text-[#0D1A63]">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-6 gap-4">
        <div>
          <h1 className="text-4xl md:text-5xl font-extrabold text-[#0D1A63] mb-2 tracking-tight">
            Thư viện <span className="text-[#2845D6]">của bạn</span>
          </h1>
          <p className="text-lg text-[#5C6A9C] font-medium">Chọn một bộ từ vựng để bắt đầu học ngay.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className={`flex items-center gap-3 px-5 py-4 rounded-2xl border-2 ${streak > 0 ? 'bg-gradient-to-br from-orange-50 to-amber-50 border-orange-200' : 'bg-slate-50 border-slate-100'}`}>
          ...
          <div>
            <p className={`text-2xl font-extrabold leading-none ${streak > 0 ? 'text-orange-600' : 'text-slate-400'}`}>{streak}</p>
      ...
      {sets.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center border-2 border-slate-100 shadow-sm mt-8">
          ...
          <h3 className="text-2xl font-extrabold text-[#0D1A63] mb-2">Chưa có bộ từ vựng nào</h3>
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="relative">
                      ...
                      {openMenuId === set.id && (
                        <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-[0_10px_25px_rgba(13,26,99,0.15)] border border-slate-100 py-2 z-[50] animate-fade-in">
                          <button
                            className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-[#0D1A63] hover:bg-[#E8EBF5] transition-colors"
                            onClick={(e) => { e.stopPropagation(); setEditingSet(set); setNewName(set.name); setOpenMenuId(null); }}
                          >
                            <Settings size={18} className="text-[#2845D6]" /> Đổi tên bộ từ vựng
                          </button>
                          <button
                            className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-[#0D1A63] hover:bg-[#E8EBF5] transition-colors"
                            onClick={(e) => { e.stopPropagation(); setManagingSet(set); setOpenMenuId(null); }}
                          >
                          ...
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); handleDeleteSet(set.id); setOpenMenuId(null); }}
                          >
                  ...
                  <h3 className="text-xl font-extrabold text-[#0D1A63] mb-2 line-clamp-1">{set.name}</h3>
          ...
          <div className="bg-white w-full max-w-md rounded-[28px] shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleRenameSet} className="p-6">
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setEditingSet(null)} className="flex-1 py-3 font-bold text-slate-500 bg-slate-100 rounded-2xl hover:bg-slate-200">Hủy</button>
                <button type="submit" disabled={loading} className="flex-1 py-3 font-bold text-white bg-[#2845D6] rounded-2xl shadow-[0_4px_0_#0D1A63] active:translate-y-1 active:shadow-none transition-all disabled:opacity-50">
