...
interface ManageWordsModalProps {
  set: any;
  isOpen: boolean;
  onClose: () => void;
}

export default function ManageWordsModal({ set, isOpen, onClose }: ManageWordsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0D1A63]/70 z-[100] flex items-center justify-center p-4 animate-fade-in backdrop-blur-md overflow-hidden text-[#0D1A63]">
            ...
            <div className="flex flex-col gap-5">
              ...
              {words.map((item, idx) => (
                ...
                >
                    ...
                    <div className="flex-[1.5] md:px-8 flex flex-col gap-1">
                        <p className="text-lg font-extrabold text-[#0D1A63]">{item.meaning}</p>
                        {item.example_en && <p className="text-sm italic text-slate-400">"{item.example_en}"</p>}
                        {item.example_vi && <p className="text-xs text-slate-300 font-medium">{item.example_vi}</p>}
                        {item.notes && (
                          <div className="mt-1 bg-[#F0F4FF] px-3 py-1.5 rounded-lg text-xs font-medium text-[#2845D6] border border-[#2845D6]/10 inline-block w-fit max-w-full">
                            💡 {item.notes}
                          </div>
                        )}
                    </div>
